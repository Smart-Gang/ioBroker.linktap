...
class LinkTapTapLinker {
    /**
     * @param {object} options - taplinker initialization options
     */
    constructor(options) {
    ...
    async queryWateringStatus() {
        var requestData = `{"username": "${this.username}", "apiKey": "${this.apiKey}", "taplinkerId": "${
            this.taplinkerId
        }"}`;
        await fetch(this.urlGetWateringStatus, {
            method: "POST",
    ...
    activateInstantMode(action, duration, autoBack) {
        if (action === true) {
            if (duration < 1 || duration > 1439) {
                this.logger.error(
                    "Invlaid InstantModeDuration value. Duration range is from 1 minute to 1439 minutes. No mode was set.",
                );
                return;
            }
        }
        var requestData = `{"username": "${this.username}", "apiKey": "${this.apiKey}", "taplinkerId": "${
            this.taplinkerId
        }", "gatewayId": "${this.gatewayId}", "action": "${action}", "duration": "${duration}", "autoBack": "${
            autoBack
        }", "eco": "false"}`;
        fetch(this.urlActivateInstantMode, {
            ...
            .then(res => {
                this.logger.info(
                    `Sent instant mode command with action: ${action} and duration: ${duration}. Status: ${res.status}`,
                );
                return res.json();
            })
            .then(json => {
    ...
    activateEcoInstantMode(action, duration, autoBack, ecoOn, ecoOff) {
        if (action === true) {
            ...
            if (ecoOn === null || ecoOff === null) {
                this.logger.error(
                    "Invlaid EcoInstantModeOn /EcoInstantModeOff value(s). Duration range is from 1 minute to 1439 minutes. No mode was set.",
                );
                return;
            }
            if (ecoOn >= duration) {
                this.logger.error(
                    "Invalid EcoInstantModeOn value. The value need to be less than InstantModeDuration. No mode was set.",
                );
                return;
            }
        }
        var requestData = `{"username": "${this.username}", "apiKey": "${this.apiKey}", "taplinkerId": "${
            ...
            autoBack
        }", "eco": "true", "ecoOn": "${ecoOn}", "ecoOff": "${ecoOff}"}`;
        fetch(this.urlActivateInstantMode, {
            method: "POST",
            headers: this.headers,
            body: requestData,
